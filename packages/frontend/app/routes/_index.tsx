...
export default function Index() {
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", lineHeight: "1.8" }} className="h-full">
      <section className="bg-gradient-to-r from-slate-900 to-slate-700 h-96 flex justify-center items-center text-3xl text-white rounded-r-md">
        <p>Hello There! My name is <span className="font-bold">yupix</span></p>
      </section>
      <Spacer size="xl" />
      <section>
        ...
        <div className="flex gap-4">
          {blogs.data.map((blog) => (
            <Link key={blog.id} to={`/posts/${blog.id}`} unstable_viewTransition className="grow-0 w-1/3">
              <PostCard post={blog} />
            </Link>
          ))}
        </div>
